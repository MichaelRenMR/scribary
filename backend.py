...
def upload(pdf_file, title='untitled', desc='desc', school='umass', courseid='0000'):
  ...
  s3.put(lpath, rpath)
  os.remove(lpath)
  # with s3.open(rpath, 'wb') as s3file:
  #   s3file.write(pdf_file.read())
  #   s3file.flush()
  #   s3file.close()

  # Edit metadata
  s3.setxattr(rpath, copy_kwargs={'ContentType': 'application/pdf', 'ContentDisposition': 'inline'})
